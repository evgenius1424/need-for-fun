...
- Movement/collision physics now has one Rust source of truth in `shared/physics_core`.
- Server uses it natively through `physics_core` dependency.
- Client uses it through WASM (`src/engine/wasm/physics_core.js` generated by `npm run build:wasm`).
- Detailed integration docs: `docs/shared-rust-physics-kernel.md`.
